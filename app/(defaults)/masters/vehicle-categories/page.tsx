...
const VehicleCategoriesMaster = () => {
    const currentUser = useSelector((state: IRootState) => state.auth.user);
    const isOwner = currentUser?.role?.toLowerCase() === 'owner';

    const activeTab = 'vehicle-categories';
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (json.success) {
                showToast('Vehicle category deleted successfully!', 'success');
                fetchData();
            }
        } catch (error) {
            console.error(error);
            showToast('Error deleting vehicle category', 'error');
        } finally {
            setDeleteId(null);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <ul className="flex space-x-2 rtl:space-x-reverse mb-6">
                ...
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2 font-bold text-white-dark uppercase tracking-widest text-[10px]"><span>Vehicle Categories</span></li>
            </ul>
                ...
                <div className="panel shadow-2xl rounded-2xl border-none">
                    <div className="mb-8 flex items-center justify-between border-b border-[#ebedf2] dark:border-[#1b2e4b] pb-5">
                        <div className="flex items-center">
                            ...
                            <div>
                                <h5 className="text-xl font-black text-black dark:text-white-light uppercase tracking-tight">{editItem ? 'Edit Vehicle Category' : 'Create New Vehicle Category'}</h5>
                                <p className="text-primary text-xs font-bold uppercase tracking-widest mt-1 opacity-70">Configuration Portal: Vehicle Categories Master</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form className="max-w-5xl mx-auto space-y-10" onSubmit={handleAdd}>
                        <div className="grid grid-cols-1 gap-8">
                            <div>
                                <label className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3 block">வகை பெயர் (Vehicle Category Name)</label>
                                <input
                                    type="text"
                        ...
                        <table className="table-hover">
                            <thead>
                                <tr className="!bg-primary/5">
                                    <th className="font-black uppercase tracking-widest text-[10px] py-4">Category Name</th>
                            ...
                            <tbody className="font-bold">
                                {loading ? (
                                    ...
                                    <tr><td colSpan={3} className="text-center py-20 text-white-dark uppercase font-black tracking-widest text-sm opacity-20">No vehicle categories found</td></tr>
                                ) : (
                                    data.map((item: any) => (
                                        <tr key={item._id} className="group hover:bg-primary/5 transition-all">
                                            ...
                                            <td className="py-4 text-white-dark">{item.description || '-'}</td>
...
export default VehicleCategoriesMaster;
