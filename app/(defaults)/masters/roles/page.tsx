...
const RoleMaster = () => {
    ...
    const handleAdd = async (e: any) => {
        ...
        try {
            ...
            if (json.success) {
                showToast(editItem ? 'Role updated successfully!' : 'Role added successfully!', 'success'); setNewItem({
                    name: '',
                    description: ''
                });
                setEditItem(null);
    ...
    return (
        <div className="space-y-6">
            <ul className="flex space-x-2 rtl:space-x-reverse mb-6">
                ...
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2 font-bold text-white-dark uppercase tracking-widest text-[10px]"><span>User Roles</span></li>
            </ul>
                ...
                <div className="panel shadow-2xl rounded-2xl border-none">
                    <div className="mb-8 flex items-center justify-between border-b border-[#ebedf2] dark:border-[#1b2e4b] pb-5">
                        <div className="flex items-center">
                            ...
                            <div>
                                <h5 className="text-xl font-black text-black dark:text-white-light uppercase tracking-tight">{editItem ? 'Edit User Role' : 'Create New User Role'}</h5>
                                <p className="text-primary text-xs font-bold uppercase tracking-widest mt-1 opacity-70">Configuration Portal: Role Master</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form className="max-w-5xl mx-auto space-y-10" onSubmit={handleAdd}>
                        <div className="grid grid-cols-1 gap-8">
                            <div>
                                <label className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3 block">Role Name (பதவிப் பெயர்)</label>
                                <input
                                    type="text"
                ...
                <div className="panel shadow-lg rounded-2xl border-none">
                    <div className="mb-5 flex items-center justify-between">
                        ...
                        <button type="button" className="btn btn-primary shadow-[0_10px_20px_rgba(67,97,238,0.3)] rounded-xl py-2.5 px-6 font-black uppercase tracking-widest text-[10px]" onClick={() => {
                            ...
                            <IconPlus className="mr-2 w-4 h-4" /> Add Role
                        </button>
                    </div>
                        ...
                        <table className="table-hover">
                            <thead>
                                <tr className="!bg-primary/5">
                                    <th className="font-black uppercase tracking-widest text-[10px] py-4 w-1/3">Role Name</th>
                            ...
                            <tbody className="font-bold">
                                {loading ? (
                                    ...
                                    <tr><td colSpan={3} className="text-center py-20 text-white-dark uppercase font-black tracking-widest text-sm opacity-20">No roles found</td></tr>
                                ) : (
                                    data.map((item: any) => (
