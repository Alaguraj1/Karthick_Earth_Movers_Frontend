...
const ExplosiveMaterialsMaster = () => {
    const currentUser = useSelector((state: IRootState) => state.auth.user);
    const { showToast } = useToast();
    const isOwner = currentUser?.role?.toLowerCase() === 'owner';

    const activeTab = 'explosive-materials';
    ...
    return (
        <div className="space-y-6">
            <ul className="flex space-x-2 rtl:space-x-reverse mb-6">
                ...
                <li className="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2 font-bold text-white-dark uppercase tracking-widest text-[10px]"><span>Explosive Materials</span></li>
            </ul>
                ...
                <div className="panel shadow-2xl rounded-2xl border-none">
                    <div className="mb-8 flex items-center justify-between border-b border-[#ebedf2] dark:border-[#1b2e4b] pb-5">
                        <div className="flex items-center">
                            ...
                            <div>
                                <h5 className="text-xl font-black text-black dark:text-white-light uppercase tracking-tight">{editItem ? 'Edit Material' : 'Add New Explosive Material'}</h5>
                                <p className="text-primary text-xs font-bold uppercase tracking-widest mt-1 opacity-70">Configuration Portal: Explosive Materials Master</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form className="max-w-5xl mx-auto space-y-10" onSubmit={handleAdd}>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div className="md:col-span-3">
                                <label className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3 block">பொருள் பெயர் (Material Name)</label>
                                <input
                                    type="text"
                        ...
                        <table className="table-hover">
                            ...
                            <tbody className="font-bold">
                                {loading ? (
                                    ...
                                    <tr><td colSpan={4} className="text-center py-20 text-white-dark uppercase font-black tracking-widest text-sm opacity-20">No materials found</td></tr>
                                ) : (
                                    data.map((item: any) => (
            ...
            <DeleteConfirmModal
                ...
                title="Delete Explosive Material"
...
export default ExplosiveMaterialsMaster;
